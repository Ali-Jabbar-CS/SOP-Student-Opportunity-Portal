...
export default function Tracker() {
  ...
  const [newApp, setNewApp]     = useState({ title: '', org: '', status: 'interested', deadline: '', notes: '' })
  ...
  const moveCard = async (card, newStatus) => {
    ...
    setColumns(prev => {
      ...
      updated[newStatus] = [{ ...card, status: newStatus }, ...updated[newStatus]]
      return updated
    })
  }
  ...
  const addApplication = async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        notes:    `${newApp.org}||${newApp.deadline}||${newApp.notes}`,
        progress: 10,
      })
      .select()
      .single()
  ...
  const parseCard = (card) => {
    ...
    return {
      ...card,
      title:    card.title    || parts[0] || 'Untitled Application',
      org:      card.org      || parts[0] || 'Unknown Org',
      deadline: card.deadline || parts[1] || '',
      note:     parts[2] || '',
    }
  }
                ...
                {cards.map(rawCard => {
                  ...
                  return (
                    <div key={card.id} style={{
                      ...
                      <div style={{ fontFamily: 'Sora, sans-serif', fontSize: 12, fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>
                        {card.title}
                      </div>
                      <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 8 }}>{card.org}</div>
                      {card.deadline && (
                      ...
                      <div style={{ height: 3, borderRadius: 2, background: 'var(--border)', overflow: 'hidden', marginBottom: 8 }}>
                        <div style={{
                          ...
                          background: colKey === 'results' ? meta.color : 'var(--green)',
