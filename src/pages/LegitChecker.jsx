...
const SAFE_CHECKS = [
  { s: 'pass', title: 'Organization verified',        detail: 'This organization has an official website and matches known STEM opportunity databases.' },
  { s: 'pass', title: 'No application fee required',  detail: 'The posting does not ask for payment at any stage of the application process.' },
  { s: 'pass', title: 'Clear contact information',    detail: 'Official domain email and phone number are provided and verified.' },
  { s: 'pass', title: 'Consistent deadline & details', detail: "Deadline and award amounts match the organization's official program page." },
  { s: 'warn', title: 'Limited online reviews',       detail: 'Fewer than 10 student reviews found. Consider asking your advisor for confirmation.' },
]

const SCAM_CHECKS = [
  { s: 'fail', title: 'Requires upfront payment',    detail: 'Legitimate internships and scholarships NEVER ask students to pay fees to apply.' },
  { s: 'fail', title: 'No verifiable organization',  detail: 'We could not find a matching registered organization or official website.' },
  { s: 'warn', title: 'Vague job description',       detail: 'The description lacks specific responsibilities — common in fraudulent postings.' },
  { s: 'warn', title: 'Non-corporate contact email', detail: 'The contact email uses Gmail/Yahoo rather than an official company domain.' },
  { s: 'pass', title: 'No known blacklist match',    detail: 'This URL was not found in our database of known scam sites.' },
]
...
const VERDICT_STYLES = {
  safe:   { bg: 'var(--green-light)', color: 'var(--green)', label: 'Looks Legitimate',  sub: 'Organization verified, no red flags found' },
  danger: { bg: 'var(--red-light)',   color: 'var(--red)',   label: 'Likely Scam',        sub: 'Multiple red flags detected' },
}
  ...
  const check = () => {
    ...
    setTimeout(() => {
      const isScam =
        ...
        input.toLowerCase().includes('wire') ||
        input.toLowerCase().includes('send money')
      setChecking(false)
      setResult(isScam
        ? { score: 24, verdict: 'danger', checks: SCAM_CHECKS }
        : { score: 93, verdict: 'safe',   checks: SAFE_CHECKS }
      )
    }, 2000)
  }
  ...
  return (
    <div style={{ padding: '26px 30px' }}>
      ...
      <p style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 24, lineHeight: 1.6 }}>
        Paste an opportunity description or URL — our AI scans for red flags like fake scholarships,
        scam internships, and suspicious postings that target international students.
      </p>
          ...
          <div style={{
            ...
            <div style={{ padding: '18px 20px' }}>
              ...
              <textarea
                ...
                placeholder={'Paste the opportunity description or email here...\n\nExample: "Earn $500/week as a STEM intern. Send $50 application fee to get started..."'}
                style={{
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 13, fontWeight: 700, cursor: checking || (!input && !url) ? 'not-allowed' : 'pointer',
          ...
          <div>
            ...
            {RECENT.map((r, i) => (
              <div key={i} style={{
                ...
                background: 'var(--surface3)', marginBottom: 7, cursor: 'pointer',
              }}>
                <div style={{
                  width: 28, height: 28, borderRadius: 8, flexShrink: 0, fontSize: 13,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  background: r.verdict === 'safe' ? 'var(--green-light)' : 'var(--red-light)',
                  color: r.verdict === 'safe' ? 'var(--green)' : 'var(--red)',
                }}>{r.verdict === 'safe' ? '✓' : '✗'}</div>
                <div style={{ flex: 1 }}>
        ...
        <div style={{
          ...
          {checking ? (
            <div style={{
              ...
              Scanning organization, checking databases…
            </div>
            ...
            <>
              ...
              <div style={{
                ...
                borderBottom: `1px solid ${VERDICT_STYLES[result.verdict].color}22`,
              }}>
                <div style={{
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: VERDICT_STYLES[result.verdict].color }}>
                  {VERDICT_STYLES[result.verdict].label}
                </div>
                <div style={{ fontSize: 11, color: VERDICT_STYLES[result.verdict].color, opacity: 0.75, marginTop: 3 }}>
                  {VERDICT_STYLES[result.verdict].sub}
                </div>
              </div>
                ...
                {result.checks.map((c, i) => (
                  <div key={i} style={{
                    ...
                    <div style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      background: STATUS_STYLES[c.s].bg,
                      color: STATUS_STYLES[c.s].color,
                      fontSize: 11,
            ...
            <div style={{
              ...
              <div style={{ fontSize: 12, color: 'var(--text2)', maxWidth: 260, lineHeight: 1.6, marginBottom: 20 }}>
                Drop in any scholarship email, LinkedIn post, or job description. Our AI cross-references it
                against verified databases and known scam patterns.
              </div>
