...
export default function OppCard({ opp, onShare, onCoverLetter }) {
  ...
  return (
    <div style={{
      background: 'var(--surface)', border: `1.5px solid ${added ? 'var(--green)' : 'var(--border)'}`,
      borderRadius: 16, padding: 18, transition: 'all 0.2s', position: 'relative',
    }}
      onMouseEnter={e => { if (!added) { e.currentTarget.style.borderColor = 'var(--card-hover-border)'; e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = 'var(--shadow-md)' }}}
      onMouseLeave={e => { if (!added) { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none' }}}
    >
      {/* Added to tracker banner */}
      {added && (
        <div style={{
              ...
              {[
                ...
                <button key={s.k} onClick={() => setStatus(s.k)}
                  style={{
                    ...
                    border: `1.5px solid ${status === s.k ? s.color : 'var(--border)'}`,
